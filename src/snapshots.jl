...
# Netgen.GetNV counts corner vertices, so it is 4 (tet) / 3 (triangle) for both linear
...
tetrahedron, `Netgen.GetNV == 4`). Returns `false` for mixed/non-simplex meshes and
...
function supported_snapshot_topology(m)
    ...
    if d == 3
        for i in 1:Netgen.GetNE(m)
            Netgen.GetNV(Netgen.VolumeElement(m, i)) == 4 || return false
        end
        for i in 1:Netgen.GetNSE(m)
            Netgen.GetNV(Netgen.SurfaceElement(m, i)) == 3 || return false
        end
        return true
    elseif d == 2
        for i in 1:Netgen.GetNSE(m)
        ...
        for i in 1:Netgen.GetNSeg(m)
            Netgen.GetNP(Netgen.LineSegment(m, i)) >= 2 || return false
        end
        return true
    else
        return false
